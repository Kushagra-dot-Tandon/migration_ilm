...
def encrypt_values(data, encrypter=None, skip_keys=None):
    ...
    if data is None:
        data = ""
        return encrypter(data.encode('unicode_escape')).decode('utf-8')
    ...
    if isinstance(data, int) or isinstance(data, float):
        ...
        return encrypter(data.decode('utf-8').encode('unicode_escape')).decode('utf-8')

    return encrypter(
        string_utils.get_bytes(json.dumps(data, cls=get_encoder_class()))
    )
